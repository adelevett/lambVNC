...
### 3.3 TightVNC Loopback Lockdown

The SSH reverse tunnel connects to TightVNC from `127.0.0.1` on the **sender** machine — the local end of the `-R` forward. This means TightVNC can and should be configured to **accept only loopback connections** (`Allow only loopback connections` in TightVNC → Configuration → Access Control).

With this setting enabled, TightVNC is completely unreachable from the LAN. Any VNC access — from any machine, including the monitoring station — must go through the LambVNC SSH tunnel. This hardens the threat model significantly: a compromised or misconfigured firewall, or a machine moved to a different network segment, cannot expose raw VNC traffic. The tunnel is the only path.

This is the correct production configuration. "Allow loopback connections" (without "only") is weaker: it still permits direct LAN VNC connections, which bypasses the SSH encryption and LambVNC's authentication entirely.

### 3.4 The IPv4/IPv6 Loopback Pitfall
